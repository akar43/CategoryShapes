function [E_z, E_zz] = estep_compute_Z_distr(P, S_bar, V, RR, c, Tr, sigma_sq)
...
for t=1:T,
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   %%%%%%%%%%%%%%%%%%%%% Changed Code here !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   %R_t = [RR(t,:); RR(t+T,:)]; % also renamed R_t to G_t in coed below
   G_t = c(t,1)*[RR(t,:); RR(t+T,:)];
   
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   
   for kk = 1:K,      
      M_t(1:J, kk) = (G_t(1,:)*V(1+(kk-1)*3:kk*3, :))'; 
      M_t(J+1:end, kk) = (G_t(2,:)*V(1+(kk-1)*3:kk*3, :))';
   end
   P_hat_t(1:J) = (G_t(1,:)*S_bar)'; 
   P_hat_t(J+1:end) = (G_t(2,:)*S_bar)';            
   
   %beta_t = M_t' * inv(M_t*M_t' + sigma_sq*eye(2*J));                      % (Eq 16)
   % Can be computed much more efficiently using the matrix inversion lemma:   
   AA = M_t./sigma_sq; 
   beta_t = M_t'*(invSigmaSq_p - AA*((eye(K) + M_t'*M_t./sigma_sq)\AA'));   % (Eq 16)
